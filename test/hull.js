...
module.exports = function() {
    it('should return correct hull of the "?" symbol (clockwise, default)', function() {
        ...
        // Explicitly test with clockwise=true (default)
        assert.deepEqual(hull(points, 50, undefined, true), expected);
    });

    it('should return only outer hull of the polygon with hole (clockwise, default)', function() {
    ...
    it('should return concave hull with lngs and lats (clockwise, default)', function() {
        let points,
            expected;
        ...
        // Explicitly test with clockwise=true (default)
        assert.deepEqual(hull(points, 0.0011, ['.lng', '.lat'], true), expected);
        ...
        // Explicitly test with clockwise=true (default)
        assert.deepEqual(hull(points, 10, ['.lng', '.lat'], true), expected);
    });
    ...
    it('should return hull in anti-clockwise order when clockwise=false', function() {
        // Simple test case: a square
        const points = [[0, 0], [10, 0], [10, 10], [0, 10], [5, 5]];
        const clockwiseResult = hull(points, 50, undefined, true); // default
        const antiClockwiseResult = hull(points, 50, undefined, false);
        
        // Anti-clockwise result should have reversed order (excluding the closing point)
        ...
        // Verify that anti-clockwise result is different from clockwise
        ...
        // Verify anti-clockwise order by checking that points are in reverse order
        // (excluding first point which should be the same)
        const clockwiseReversed = [clockwiseWithoutLast[0]].concat(clockwiseWithoutLast.slice(1).reverse());
        assert.deepEqual(antiClockwiseWithoutLast, clockwiseReversed, 'Anti-clockwise should be reverse of clockwise');
    });
}
