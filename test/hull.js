...
module.exports = function() {
    it('should return correct hull of the "?" symbol (anti-clockwise, default)', function() {
        ...
        // Explicitly test with clockwise=false (default)
        assert.deepEqual(hull(points, 50, undefined, false), expected);
    });

    it('should return only outer hull of the polygon with hole (anti-clockwise, default)', function() {
    ...
    it('should return concave hull with lngs and lats (anti-clockwise, default)', function() {
        let points,
            expected;
        ...
        // Explicitly test with clockwise=false (default)
        assert.deepEqual(hull(points, 0.0011, ['.lng', '.lat'], false), expected);
        ...
        // Explicitly test with clockwise=false (default)
        assert.deepEqual(hull(points, 10, ['.lng', '.lat'], false), expected);
    });
    ...
    it('should return hull in clockwise order when clockwise=true', function() {
        // Simple test case: a square
        const points = [[0, 0], [10, 0], [10, 10], [0, 10], [5, 5]];
        const antiClockwiseResult = hull(points, 50, undefined, false);
        const clockwiseResult = hull(points, 50, undefined, true);
        
        // Clockwise result should have reversed order (excluding the closing point)
        // The first point should be the same, but the order should be reversed
        assert.equal(antiClockwiseResult.length, clockwiseResult.length, 'Results should have same length');
        
        // Verify that clockwise result is different from anti-clockwise
        // (they should be reversed versions of each other)
        const antiClockwiseWithoutLast = antiClockwiseResult.slice(0, -1);
        const clockwiseWithoutLast = clockwiseResult.slice(0, -1);
        
        // Check that the order is reversed (first point stays same, rest reversed)
        assert.equal(antiClockwiseWithoutLast[0][0], clockwiseWithoutLast[0][0], 'First point should be the same');
        assert.equal(antiClockwiseWithoutLast[0][1], clockwiseWithoutLast[0][1], 'First point should be the same');
        
        // Verify clockwise order by checking that points are in reverse order
        // (excluding first point which should be the same)
        const antiClockwiseReversed = [antiClockwiseWithoutLast[0]].concat(antiClockwiseWithoutLast.slice(1).reverse());
        assert.deepEqual(clockwiseWithoutLast, antiClockwiseReversed, 'Clockwise should be reverse of anti-clockwise');
    });
}
